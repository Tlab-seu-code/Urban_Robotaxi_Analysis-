...
def plot_trip_scatter(csv_file, boundary, now, sample_fraction=0.1):
    """
    ...
    # 3. Prepare data
    origins = df[['lon_wgs84', 'lat_wgs84']].values
    ...
    # 12. Save and show
    plt.savefig("..\\dataV3\\poi\\classified_poi\\"+now+'.svg', dpi=300, bbox_inches='tight')
...
#     now = 'work'
#     # Plot scatter
#     csv_file = "..\\dataV3\\poi\\classified_poi\\"+now+".csv"  # 替换为实际文件路径
#     plot_trip_scatter(csv_file, boundary, now, sample_fraction=0.1)  # 使用10%的数据样本
...
def plot_vehicle_heatmap_english(csv_file, boundary, now):
    """
    ...
    valid_df = df[(df['lon_wgs84'] >= min_lon) & (df['lon_wgs84'] < max_lon) &
                  (df['lat_wgs84'] >= min_lat) & (df['lat_wgs84'] < max_lat) #&
    ...
    for _, row in valid_df.iterrows():
        x, y = get_grid_xy(row['lon_wgs84'], row['lat_wgs84'])
    ...
    # 16. Save and display
    plt.savefig("..\\dataV3\\poi\\classified_poi\\"+now+'2.svg', dpi=300, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    csv_file = "..\\dataV3\\poi\\classified_poi\\"+now+".csv"  # 替换为实际文件路径
    plot_vehicle_heatmap_english(csv_file, boundary, now)
