...
OUT_PNG = os.path.join(OUT_DIR, "Robotaxi_14_CTX_vs_SNE2.png")
...
# CCI quantiles
CCI_QUANTILES = np.linspace(1 / 15, 14 / 15, 13)

# =========================
# Robotaxi 14-point series
# =========================
ROBOTAXI_14 = {
    "orders": None,
    "bin_label": [
        "<6.7%", "6.7–13.9%", "13.9–21.1%", "21.1–28.3%", "28.3–35.6%", "35.6–42.8%",
        "42.8–50.0%", "50.0–57.2%", "57.2–64.4%", "64.4–71.7%", "71.7–78.9%",
        "78.9–86.1%", "86.1–93.3%", ">93.3%"
    ],
}

CUTS_14 = [0, 0.067, 0.139, 0.211, 0.283, 0.356, 0.428, 0.500, 0.572, 0.644, 0.717, 0.789, 0.861, 0.933, 1.01]
...
def bin_counts_to_14_by_cci_quantiles(
    ...
    Match orders.py interval rules:
      intervals = [(-inf, q0], (q0, q1], ..., (q12, +inf]]
      mask: (cci > low) & (cci <= high)
    Equivalent mapping:
      idx = searchsorted(q_values, cci, side="left")  -> 0..13
    """
    ...
    for (gx, gy), n in grid_counts.items():
        ...
        if cci is None or not np.isfinite(cci):
            continue
        idx = int(np.searchsorted(q, float(cci), side="left"))  # 0..13
        if idx < 0:
            idx = 0
        elif idx > 13:
            idx = 13
...
def compute_robotaxi_orders_14(av_csv: str, cci_file: str) -> np.ndarray:
    grid_cci, q_values = load_cci_quantile_values(cci_file)
    grid_counts = read_robotaxi_grid_counts(av_csv)
    return bin_counts_to_14_by_cci_quantiles(grid_counts, grid_cci, q_values)
...
def load_grid_features_14bins(cuts: List[float]) -> pd.DataFrame:
    # --- Population ---
...
def plot_robotaxi_ctx_vs_sne(
    ...
    # Peak positions in the mid-range
    mid_idxs = mid_range_indices(cuts, MID_CCI_LOW, MID_CCI_HIGH)
    i_ctx = argmax_in_window(y_ctx, mid_idxs)
    i_sne = argmax_in_window(y_sne, mid_idxs)

    # Vertical guide lines
    ax.axvline(i_ctx, color=C_CTX, linestyle=(0, (3, 2)), linewidth=1.0, alpha=0.65, zorder=4)
    ax.axvline(i_sne, color=C_SNE, linestyle=(0, (3, 2)), linewidth=1.0, alpha=0.65, zorder=4)

    # Peak markers
    ax.scatter([i_ctx], [y_ctx[i_ctx]], s=36, facecolors="white", edgecolors=C_CTX, linewidths=1.1, zorder=7)
    ax.scatter([i_sne], [y_sne[i_sne]], s=36, facecolors="white", edgecolors=C_SNE, linewidths=1.1, zorder=7)

    # Peak labels
    ax.text(i_ctx - 0.10, y_ctx[i_ctx], "mid-peak (CTX) ", color=C_CTX, fontsize=7, va="bottom", ha="right")
    ax.text(i_sne + 0.10, y_sne[i_sne], " mid-peak (SNE)", color=C_SNE, fontsize=7, va="bottom", ha="left")
    ...
    ax.text(x[j] + 0.10, y_high[j], " max Δ", fontsize=7, color="0.35", va="bottom", ha="left")
    ...
    ax.legend(frameon=False, loc="upper left", ncol=1, handlelength=2.6, borderaxespad=0.2)
    ...
    stats_text = [
        f"CTX (POP + CCI + POI): R²={r2_ctx:.3f}, r={r_ctx:.3f}",
        f"POP + SNE: R²={r2_sne:.3f}, r={r_sne:.3f}",
    ]
    ax.text(
        0.02, 0.60,
        "\n".join(stats_text),
...
def main():
    # 1) compute observed robotaxi orders (14 bins) and fill ROBOTAXI_14
    ...
    orders_14 = compute_robotaxi_orders_14(av_csv=av_csv, cci_file=cci_csv)
    ROBOTAXI_14["orders"] = orders_14.astype(int).tolist()

    # 2) proceed with original logic
    df = pd.DataFrame(ROBOTAXI_14)
    y_obs = df["orders"].values.astype(float)
    x_labels = df["bin_label"].tolist()

    feat = load_grid_features_14bins(CUTS_14)
    ...
    print("\n[Observed Robotaxi orders by CCI-quantile bins (14)]")
    print(ROBOTAXI_14["orders"])
