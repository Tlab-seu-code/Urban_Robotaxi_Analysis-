...
try:
    import sumolib
except ImportError:  # 允许在无SUMO环境下运行DW逻辑
    sumolib = None
import csv
...
# --- 文件路径和文件夹创建 ---
traj_file = "v12-traj.csv"  # 修改为CSV文件
...
# --- 全局变量定义 ---
WALKING_RADIUS_METERS = 500    # 步行半径 (500 米)
MAX_WAIT_TIME = 300            # 最大等待时间 (5 分钟)
DEFAULT_MAX_CAPACITY = 3       # 出租车最多载客数
...
def is_workday(date_obj):
    """判断是否为工作日"""
    return date_obj.weekday() < 5  # 0-4 是工作日
...
class DynamicWaitingRideSharing:
    def __init__(
        ...
        self.waiting_orders: List[Dict] = []  # 等待中的订单
        self.matched_pairs: List[Dict] = []   # 已匹配的订单对
    ...
    def process_new_order(self, new_order: Dict) -> Optional[Dict]:
        ...
        for waiting_order in time_window_orders:
            ...
            if quick_distance > self.max_wait_distance_meters * 1.5:  # 放宽预筛选条件
                continue
...
def main():
    ...
    try:
        ...
        for i, order in enumerate(orders):
            ...
            # 检查等待中的订单是否超时
            expired_orders = dw_system.check_waiting_orders(order['call_time'])
            
            # 处理新订单
            match_result = dw_system.process_new_order(order)
