...
# 文件路径
traj_file = "v12-traj_sample_2024-11-12.csv"
speed_file = "speed_filled_step1_sample_2024-11-12.csv"
network_file = "robust.net.xml"
output_folder = "carpooling_one_day_results_v12_sumo"
os.makedirs(output_folder, exist_ok=True)

# 全局常量
MAX_IDLE_TIME_SECONDS = 7200  # 最大允许空闲时间 (2 小时)
DEFAULT_TRANSFER_SPEED_MPS = 8.0  # 直线距离模式下的默认调度速度
DEFAULT_TARGET_DATE = "2024/11/12"  # Nature sample 默认只跑一天


def normalize_date_str(date_str: str) -> str:
    """Normalize date strings to the v12 order-data format: YYYY/M/D."""
    if date_str is None:
...
# 灵敏度分析参数：动态等待时间（分钟）与空间（米）
SENSITIVITY_CONFIGS = [
    # {"name": "w3m_d300m_sumo", "dw_max_wait_minutes": 3, "dw_max_distance_m": 300, "use_sumo_paths": True},
    # {"name": "w5m_d500m_sumo", "dw_max_wait_minutes": 5, "dw_max_distance_m": 500, "use_sumo_paths": True},
    # {"name": "w8m_d800m_sumo", "dw_max_wait_minutes": 8, "dw_max_distance_m": 800, "use_sumo_paths": True},
...
def is_workday(date_str):
    ...
    date = datetime.strptime(date_norm, "%Y/%m/%d")
...
def load_orders(target_dates):
    """加载订单数据；target_dates 为 YYYY/M/D 格式日期列表。"""
    print("加载订单数据...")
    orders_by_date = {}
    skipped_count = 0
    ...
    target_dates = [normalize_date_str(d) for d in target_dates]
    ...
    with open(traj_file, "r", encoding="utf-8-sig") as csvfile:
        ...
        for row in reader:
            ...
            # 对于v12数据，从"到达起点时间"字段提取日期
            if "到达起点时间" in row and row["到达起点时间"]:
                order_date = normalize_date_str(row["到达起点时间"].split(" ")[0])
            else:
                date_skipped += 1
                continue
...
def main():
    ...
    parser.add_argument("--dw-only", action="store_true", help="仅运行DW拼单，不运行二部图")
    parser.add_argument("--target-date", default=DEFAULT_TARGET_DATE, help="只处理一天数据；格式 YYYY/M/D 或 YYYY-MM-DD，默认 2024/11/12")
    args = parser.parse_args()
    target_date = normalize_date_str(args.target_date)
    target_dates = [target_date]

    print(f"开始处理v12样本数据（只处理 {target_date} 一天，改进版集成动态拼单 - SUMO路径版）...")
    ...
    if not args.dw_only:
        ...
        with open(speed_file, 'r', encoding='utf-8-sig', newline='') as f:
    ...
    print(f"将处理目标日期: {target_date}")
    
    # 加载目标日期数据
    orders_by_date = load_orders(target_dates)
    ...
    for scenario_config in SENSITIVITY_CONFIGS:
        ...
        for batch_start in range(0, len(target_dates), batch_size):
            ...
            for date in batch_dates:
                ...
                daily_max_cars = 0
                if date == target_date and "night_early" in daily_results:
                ...
                # 每天订单服务人数统计 + 融合类型统计（基于四个主时段，避免与night_early重复）
